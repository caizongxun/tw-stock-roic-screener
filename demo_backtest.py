#!/usr/bin/env python3
"""
demo_backtest.py - 不需 FinMind Token，直接用 yfinance 跑 5 年回測示範
"""
import os
import pandas as pd
...
def run_demo():
    ...
    for sid in DEMO_STOCKS:
        try:
            df = yf.download(f"{sid}.TW", start=start_date, end=end_date,
                             progress=False, auto_adjust=True)
            if not df.empty:
                col = df[["Close"]].rename(columns={"Close": sid})
                all_prices.append(col)
                print(f"  {sid}: {len(df)} 筆")
            else:
                print(f"  {sid}: 無資料")
    ...
    bench_df  = yf.download("^TWII", start=start_date, end=end_date,
                             progress=False, auto_adjust=True)
    ...
    for sid in price_df.columns:
        ...
        rows.append({
            "stock_id":     sid,
            "total_return": round((s.iloc[-1] / s.iloc[0] - 1) * 100, 2),
            "cagr":         round(calc_annual_return(s) * 100, 2),
            "max_drawdown": round(calc_max_drawdown(s) * 100, 2),
        })
    ...
    axes[0].plot(portfolio_cum.index, portfolio_cum.values,
                 label="ROIC 優質股組合", color="#E8534B", linewidth=2)
    ...
    plt.close()
    print("\n[完成] 圖表已儲存至 output/demo_backtest_chart.png")
